...
type Props = {
    ...
    selectedDate?: string;
    memberContributionTotals: Record<
        string,
        Record<string, MemberContributionSummary>
    >;
};
...
export default function BulkContributions({
    ...
    const [contributionTypeId, setContributionTypeId] = useState(
        String(defaultType?.id ?? ''),
    );
    const [contributionChannelId, setContributionChannelId] = useState(
        String(defaultChannel?.id ?? ''),
    );
    const [date, setDate] = useState(selectedDate ?? defaultMeeting.date);
    const contributionDate = selectedDate ?? date;
    ...
    const [rows, setRows] = useState<EntryRow[]>(() =>
        members.map((m) => ({
            ...
            included: false,
            amount: defaultType?.default_amount
                ? String(defaultType.default_amount)
                : '',
    ...
    const getMemberSummary = (
        memberId: number,
    ): MemberContributionSummary | undefined =>
        memberContributionTotals[String(memberId)]?.[contributionTypeId];

    const isMemberComplete = (memberId: number): boolean =>
        getMemberSummary(memberId)?.met ?? false;

    const suggestedAmountForMember = (
        memberId: number,
    ...
    const syncRowAmounts = (amount: string) => {
        setRows((prev) =>
            prev.map((r) => {
                ...
                return {
                    ...r,
                    amount: suggestedAmountForMember(r.member_id, amount),
                };
            }),
        );
    };
    ...
    const handleDateChange = (newDate: string) => {
        ...
        router.get(
            ...
            { date: newDate },
            {
                preserveState: true,
                preserveScroll: true,
                only: ['memberContributionTotals', 'selectedDate'],
            },
        );
    };
    ...
    const includedCount = rows.filter(
        (r) => r.included && parseFloat(r.amount) > 0,
    ).length;
        ...
        () =>
            rows
                .filter(
                    (r) =>
                        !(
                            memberContributionTotals[String(r.member_id)]?.[
                                contributionTypeId
                            ]?.met ?? false
                        ),
                )
        ...
        setRows((prev) =>
            prev.map((r) =>
                r.included && !isMemberComplete(r.member_id)
                    ? {
                          ...r,
                          amount: suggestedAmountForMember(
                              r.member_id,
                              defaultAmount,
                          ),
                      }
    ...
    const handleTypeChange = (typeId: string) => {
        ...
        setRows((prev) =>
            prev.map((r) => {
                const summary =
                    memberContributionTotals[String(r.member_id)]?.[typeId];
    ...
    return (
        <>
            ...
            <PageHeader
                ...
                actions={
                    <button
                        ...
                        onClick={() => router.visit('/portal/contributions')}
                    >
            ...
            <div className="card cp-panel mb-4 border-0">
                <div className="card-body">
                    <div className="row g-3">
                        ...
                        <div className="col-md-3">
                            ...
                            <select
                                className="form-select form-select-sm"
                                value={contributionChannelId}
                                onChange={(e) =>
                                    setContributionChannelId(e.target.value)
                                }
                            >
                                {channels.map((c) => (
                        ...
                        <div className="col-md-2">
                            ...
                            <input
                                ...
                                value={defaultAmount}
                                onChange={(e) =>
                                    handleDefaultAmountChange(e.target.value)
                                }
                                placeholder={
            ...
            <div className="card cp-panel border-0">
                <div className="card-header d-flex align-items-center justify-content-between flex-wrap gap-2 border-0 bg-transparent px-3 pt-3">
                ...
                <div className="table-responsive">
                    <table className="table-sm table-hover cp-table mb-0 table align-middle">
                        <thead>
                            <tr>
                                <th style={{ width: 40 }}>
                                    <input
                                        ...
                                        onChange={(e) =>
                                            toggleAllMembers(e.target.checked)
                                        }
                                    ...
                                    >
                                        ...
                                        <td>
                                            <div className="d-flex align-items-center gap-2">
                                                ...
                                                {complete && (
                                                    <span className="badge bg-success-subtle text-success border-success-subtle border">
                                                        Paid
                                                    </span>
                                                )}
                                                {!complete &&
                                                    ...
                                                    summary.remaining !=
                                                        null && (
                                                        <span className="badge bg-warning-subtle text-warning border-warning-subtle border">
                                                            {summary.remaining}{' '}
                                                            due
                                                        </span>
                                                    )}
                ...
                <div className="card-footer d-flex justify-content-end gap-2 border-0 bg-transparent py-3">
                    <button
                        type="button"
                    ...
                    >
                        {processing
                            ? 'Saving...'
                            : `Record ${includedCount} contribution(s)`}
                    </button>
                </div>
            </div>
        </>
