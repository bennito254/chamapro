...
export default function Page({ loan, interest_outstanding, principal_outstanding }: Props) {
    ...
    return (
        <>
            ...
            <PageHeader
                ...
                actions={
                    <Link href="/portal/loans" className="btn btn-outline-secondary btn-sm">
                        Back to loans
                    </Link>
                }
            />
            ...
            <DetailCard
                ...
                fields={[
                    ...
                    { label: 'Interest', value: formatCurrency(loan.interest_amount ?? 0) },
                    { label: 'Total', value: formatCurrency(loan.total_amount ?? loan.outstanding_balance) },
                    { label: 'Repayment period', value: loan.repayment_period ? `${loan.repayment_period} months` : '—' },
                        ...
                        <Form {...repaymentRoute}>
                            {({ errors, processing }) => (
                                <>
                                    ...
                                    />
                                    <div className="mb-3">
                                        <label htmlFor="field-amount" className="form-label">
                                    ...
                                    <FormField
                                        ...
                                        error={errors.date}
                                    />
                                    <FormField label="Method" name="method" error={errors.method} placeholder="Cash, M-Pesa, bank..." />
                                    <FormField label="Reference" name="reference_number" error={errors.reference_number} />
                                    <FormField label="Notes" name="notes" type="textarea" error={errors.notes} />
                                    <button type="submit" className="btn btn-primary" disabled={processing || !amount}>
                                        {processing ? 'Recording...' : 'Record repayment'}
                                    </button>
                                </>
                            )}
                        </Form>
            ...
            <div className="card border-0 shadow-sm mt-4">
                ...
                <div className="card-body p-0">
                    <DataTable
                        columns={[
                            { key: 'date', label: 'Date', render: (row: LoanRepayment) => formatDate(row.date) },
                            {
                                key: 'interest_paid',
                                label: 'Interest',
                                className: 'text-end',
