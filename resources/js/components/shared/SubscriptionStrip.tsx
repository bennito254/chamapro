...
export default function SubscriptionStrip() {
    const { subscription } = usePage<{ subscription?: Subscription | null }>().props;
    ...
    const isWarning = subscription.status === 'expired' || subscription.status === 'suspended';

    return (
        <div className={`cp-subscription-strip ${isWarning ? 'cp-subscription-strip--warning' : ''}`}>
            <div className="cp-subscription-strip__inner">
                <div className="cp-subscription-strip__plan">
                    ...
                    <span className={`badge cp-subscription-strip__badge cp-subscription-strip__badge--${subscription.status}`}>
                        {statusLabel}
                    </span>
                </div>
                ...
                <div className="cp-subscription-strip__payment">
                    ...
                    <span className="cp-subscription-strip__label">Next payment</span>
                    <strong>{formatDate(nextPayment)}</strong>
                </div>
                {(subscription.status === 'expired' || subscription.status === 'trial') && (
                    <Link href="/portal/subscription/renew" className="btn btn-sm btn-light ms-md-auto">
                        Manage subscription
                    </Link>
                )}
            </div>
